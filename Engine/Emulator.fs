...
let mutable r = Array.init 8 (fun _ -> Array.create 14 0)
let mutable s = Array.create 12 0

let calculator (r' : int[][]) (s' : int[]) input output rom =
    let p = ref 0
    let pc, offset, ret = ref 0, ref 0, ref 0
    let carry, carry' = ref false, ref false
    let visible, state = ref false, ref ("", "", "", "", "")
        ...
        | 0b00 -> let arg = n / 16
                  match n % 16 with
                      ...
                      | 10 -> match arg with
                              ...
                              // TODO: Fix blog and github
                              | 12 -> let t = Array.copy r.[2] in move [2,3; 3,4; 4,5]; r.[5] <- t
                              | 14 -> for i in 0..13 do for j in 0..7 do r.[j].[i] <- 0
                      | 11 -> carry := !p = arg
                      | 12 -> pc := !ret
                      | 13 -> for i in 0..11 do s.[i] <- 0
                      | 15 -> p := (!p + 1) &&& 0xf
    let display () : string * string * string * string * string =
        if not !visible then "", "", "", "", "" else
            let renderDisplay i a b =
                (if b >= 8 then " "
                 elif i = 2 || i = 13 then (if a >= 8 then "-" else " ")
                 else string a) + (if b = 2 then "." else "")
            let renderStack (r : int array) =
                let r = Array.rev r
                let e = let x = 10 * r.[12] + r.[13] in if r.[11] = 9 then 100 - x else x
                let sf =
                    match r.[1..10] |> Array.rev |> Array.tryFindIndex ((<>) 0) with
                    | Some i -> 10 - i
                    | None -> 0
                let d = if e > 9 || r.[11] = 9 then 1 else e + 1
                let rend = Array.map string >> Array.fold (+) ""
                let m = rend r.[1 .. d]
                let f = if sf < d + 1 then "" else rend r.[d + 1 .. sf]
                let s = if r.[0] = 9 then "-" else ""
                let x = (e - d + 1) * if r.[11] = 9 then -1 else 1
                sprintf "%s%s.%s%s" s m f (if x <> 0 && m <> "0" then sprintf " %i" x else "")
            let revConcat = Array.rev >> Array.fold (+) ""
            let x = Array.mapi2 renderDisplay r.[0] r.[1] |> revConcat
            let rend i = renderStack r.[i]
            let y = rend 3
            let z = rend 4
            let t = rend 5
            let m = rend 6
    ...
    let rec step () =
        ...
        if state' <> !state then
            if not !init then
                ...
                if s' <> null then s <- s'
                init := true
                output (display ()) r s; state := state'
            else output state' r s; state := state'
            ...
            | Some k ->
                match List.tryFind (fst >> (=) k)
                 ['0',36; '1',28; '2',27; '3',26; '4',20; '5',19; '6',18; '7',52; '8',51; '9',50; '.',35; '-',54; '+',22
                  ...
                  | None -> halt := k = '`'
            | None -> ()
        if not !halt then step () else state'
    step ()